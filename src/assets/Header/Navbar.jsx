  ...
  return (
    <div className="py-6 shadow-sm font-primary w-full sticky top-0 dark:bg-slate-800 dark:text-white bg-white z-10  dark:shadow-md   ">
      <div className="flex justify-between px-6 items-center sm:px-12 lg:px-12 max-w-screen-2xl m-auto relative ">
        ...
        <div className={`flex gap-4 sm:text-lg z-20 `}>
          <button className={`cursor-pointer  ${bar ? "sm:text-white" : "sm:text-black" }`}  onClick={handleThemeChange}>
            {theme === "dark"  ? <MdLightMode className={` ${bar ? "sm:text-black" : "sm:text-white"}`} /> : <MdOutlineDarkMode className=""/>}
          </button>
          <button className="cursor-pointer  dark:text-white text-black" onClick={() => setbar(!bar)}>
